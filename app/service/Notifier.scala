...
class NexmoNotifier extends Notifier {
  ...
  override def sendSms(person: Person, text: String): Unit = {
...
class ConsoleNotifier extends Notifier {
  ...
  override def sendSms(person: Person, text: String): Unit = logger.info(s"Message to ${person.name}: '$text'")
}

trait Notifier {
  def sendSms(person: Person, text: String): Unit
}
